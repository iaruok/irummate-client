...
function FinalConfirmedNotice({ contact, contactErrorMessage, isLoadingContact, onShowContact, partnerName }) {
  return (
    <div className="border-t border-[#dce5f1] bg-white px-4 py-3">
      <div className="rounded-2xl bg-[#edf3fb] px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-sm font-extrabold text-fg-primary">최종확정이 완료됐어요.</p>
          ...
          <button
            type="button"
            className="min-h-9 rounded-full bg-brand-primary px-4 text-xs font-extrabold text-white disabled:opacity-60"
...
function ChatRoom() {
  ...
  const handleShowContact = useCallback(async () => {
    setConfirmErrorMessage('');
    setContactErrorMessage('');
    ...
    const receiverId = getReceiverId(room);
    if (!receiverId) {
      setContactErrorMessage('상대방 정보를 확인하지 못했어요. 채팅방 목록을 새로고침한 뒤 다시 시도해 주세요.');
      return;
    }
  ...
  }, [confirmedContact, isLoadingContact, room, showConfirmedContact]);
  ...
  return (
    <section className="flex h-dvh flex-col overflow-hidden bg-brand-background">
      <ChatRoomHeader
        ...
        onShowContact={shouldShowFinalConfirmedNotice ? handleShowContact : undefined}
      />
      ...
      {confirmModalStep && (
        ...
        >
          <div
            className="relative w-full max-w-[420px] rounded-lg bg-white p-5 shadow-[0_24px_60px_rgba(23,34,56,0.24)]"
            role="dialog"
            aria-modal="true"
          ...
          >
            <button
              type="button"
              className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full text-fg-basic-muted transition-colors hover:bg-[#edf2f8] disabled:opacity-50"
